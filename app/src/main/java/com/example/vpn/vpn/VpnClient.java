...
public class VpnClient extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.disconnect).setOnClickListener(v -> {
            startService(getServiceIntent().setAction(VpnServices.ACTION_DISCONNECT));
        });
    }
    ...
    protected void onActivityResult(int request, int result, Intent data) {
        if (result == RESULT_OK) {
            startService(getServiceIntent().setAction(VpnServices.ACTION_CONNECT));
        }
    }
    private Intent getServiceIntent() {
        return new Intent(this, VpnServices.class);
    }
}